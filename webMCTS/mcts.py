...
# expand
async def get_next_step_expand(node: treeNode, mcts_task):
    
    execute_action_list = []
    
    #! 异步处理action, 并初始化子节点
    action_return = await asyncio.gather(*[
        mcts_task.get_next_action(
            trace=node.trace, 
            state=node.state, 
            step=node.depth+1
        ) for _ in range(mcts_task.branch)
    ])
    for raw_action, execute_action in action_return:
        if execute_action not in execute_action_list:
            execute_action_list.append(execute_action)
            node.append_children(raw_action)
                 
    if not execute_action:
        node.update_reflection('<end>')
        return node
     
    #! 异步处理state, 并更新子节点状态信息
    state_return = await asyncio.gather(*[
        mcts_task.get_next_state_predict(
            state=node.state,
            action=action
        ) for action in node.children.keys()
    ])
    for index, action in enumerate(node.children.keys()):
        child: treeNode = node.children[action]
        child.update_state(state_return[index])
    
    #! 异步处理value, 并更新子节点价值信息
    value_return = await asyncio.gather(*[
    ...
    for index, action in enumerate(node.children.keys()):
        child: treeNode = node.children[action]
        child.update_value(value_return[index])
...
async def expand(node: treeNode, mcts_task):
    """
    ...
    # step two
    node = await get_next_step_expand(node, mcts_task)
    return node
...
async def executeRound(root: treeNode, mcts_task):
    ...
    if node.reflection == '<end>':
        ...
        node = await expand(node, mcts_task)
...
async def MCTS_search(mcts_task):
    ...
    if mcts_task.limit_type == 'time':
        ...
        while time.time() < timeLimit:
            ...
            flag, node, root = await executeRound(root, mcts_task)
            if flag:
                print('已找到解决方案！\n')
...
async def MCTS(mcts_task):
    root, node, finish = await MCTS_search(mcts_task)
