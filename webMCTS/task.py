...
class MCTS_Task(SearchTask):
    ...
    async def get_next_action(self, trace, state, step):
        """
            output:
    ...
    async def get_next_state_predict(self, state, action):
        """
            output:
    ...
    async def get_step_value(self, trace, state):
        """
            output:
    ...
    async def run(self):
        self.clear_cache()
        self.set_limit_type()
        root, node, finish = await MCTS(self)     # input mcts_task
