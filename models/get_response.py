...
async def get_proposal(
    ...
    if policy_model == 'deepseek-chat':
        while not response and cnt:
            response = await deepseek(prompt, model=policy_model, temperature=temperature, max_tokens=max_tokens)
            cnt -= 1
        if not response:
        ...
        while not response and cnt:
            response = await qwen(prompt, model=policy_model, temperature=temperature, max_tokens=max_tokens)
            cnt -= 1
        if not response:
            print(f'obtain<{policy_model}>response fail!\n')
            return []
...
async def get_state(
    ...
    if world_method == 'deepseek-chat':
        while not response and cnt:
            response = await deepseek(prompt, model=world_method, temperature=temperature, max_tokens=max_tokens)
            cnt -= 1
        if not response:
        ...
        while not response and cnt:
            response = await qwen(prompt, model=world_method, temperature=temperature, max_tokens=max_tokens)
            cnt -= 1
        if not response:
            print(f'obtain<{world_method}>response fail!\n')
            return []
...
async def get_value(
    ...
    if reward_model == 'deepseek-chat':
        while not response and cnt:
            response = await deepseek(prompt, model=reward_model, temperature=temperature, max_tokens=max_tokens)
            cnt -= 1
        if not response:
        ...
        while not response and cnt:
            response = await qwen(prompt, model=reward_model, temperature=temperature, max_tokens=max_tokens)
            cnt -= 1
        if not response:
            print(f'obtain<{reward_model}>response fail!\n')
            return []
